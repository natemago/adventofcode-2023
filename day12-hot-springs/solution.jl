...
function part2(record)
    ...
    for (springs, records) in record
        ...
        expanded_records = []
        for i in range(1, 5)
            expanded_springs = expanded_springs*"?"*springs
            expanded_records = vcat(expanded_records, records)
        end
        a = springs_combinations(1, expanded_records, expanded_springs)
        #a = get_arragements_count(springs, records)
        s += a
        println("->", (expanded_springs, expanded_records, a))
    end
    return s
end

@time println("Part 1: ", part1(read_input("input")))
@time println("Part 2: ", part2(read_input("test_input")))